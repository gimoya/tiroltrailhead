...
const searchResults = document.getElementById('trail-search-results');
const searchBtn = document.getElementById('trail-search-btn');

function normalize(str) {
  return str
    ? str.toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/[^\p{Letter}\p{Number} ]+/gu, '')
    : '';
}

// Debug: log normalized query and first 5 trail names
function debugNormalization(query, features) {
  const normQuery = normalize(query);
  console.log('Normalized query:', normQuery);
  features.slice(0, 5).forEach(f => {
    if (f.properties && f.properties.name) {
      console.log('Trail:', f.properties.name, '->', normalize(f.properties.name));
    }
  });
}

// Wait for trails to be loaded
let searchTrailFeatures = [];
let searchClickLayer = null;

if (typeof trails_json !== 'undefined' && typeof click_layer !== 'undefined') {
  searchTrailFeatures = trails_json.toGeoJSON().features;
  searchClickLayer = click_layer;
} else {
  const oldGetJSON = $.getJSON;
  $.getJSON = function() {
    const args = Array.from(arguments);
    const origCallback = args[1];
    args[1] = function(json) {
      if (typeof trails_json !== 'undefined' && typeof click_layer !== 'undefined') {
        searchTrailFeatures = trails_json.toGeoJSON().features;
        searchClickLayer = click_layer;
      }
    ...
    return oldGetJSON.apply(this, args);
  };
}
...
function doSearch() {
  ...
  // Debug normalization
  if (searchTrailFeatures && searchTrailFeatures.length) {
    debugNormalization(query, searchTrailFeatures);
  }

  // --- Search trails ---
  let results = [];
  if (searchTrailFeatures && searchTrailFeatures.length) {
    results = searchTrailFeatures
      .filter(f => (f.properties && f.properties.name && normalize(f.properties.name).includes(normQuery)))
      .map(f => ({
        type: 'trail',
  ...
  // --- Search Nominatim if not enough results or no trail match ---
  if (results.length < 5) {
    fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=10&countrycodes=at&viewbox=10.09,47.70,12.98,46.83&bounded=1`)
      .then(res => res.json())
      .then(nomResults => {
        nomResults
          .filter(place => normalize(place.display_name).includes(normQuery))
          .forEach(place => {
            results.push({
              type: 'nominatim',
...
function showResults(results) {
  ...
  results.forEach(item => {
    ...
    li.textContent = (item.type === 'trail' ? '🟢 ' : '🌍 ') + item.name;
    li.addEventListener('click', () => {
      if (item.type === 'trail' && item.feature && searchClickLayer) {
        let foundLayer = null;
        searchClickLayer.eachLayer(function(layer) {
          if (layer.feature && layer.feature.properties.name === item.feature.properties.name) {
            foundLayer = layer;
          }
        });
        if (foundLayer) {
      ...
      } else if (item.type === 'nominatim' && item.center) {
        map.setView(item.center, 15, { animate: true });
      }
      searchResults.style.display = 'none';
      searchInput.value = '';
    });
